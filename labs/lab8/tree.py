...
class Tree:
    ...
    def branching_factor(self) -> float:
        ...
        if self.is_empty() or self._subtrees == []:
            ...
            branchfactor, intnodes = self._countint()
        return branchfactor/intnodes
        # if self.is_empty():
        #     return 0.0
        # elif self._subtrees == []:
        #     return 1.0
        ...
        #         count += 1
        #         total += 1
        #         total += sub.branching_factor()
        # return total / count

    def _countint(self) -> tuple(int, int):
        #Return total number of branching factors and int nodes
        branch, intnodes = 0, 0
        if self._subtrees == []:
            return (0,0)
        else:
            ...
            for sub in self._subtrees:
                branch += 1
                branch += sub._countint()[0]
                intnodes += sub._countint()[1]
            return (branch, intnodes)
    ...
    def items_at_depth(self, d: int) -> list:
        ...
        if not self._root:
            ...
            for sub in self._subtrees:
                result.extend(sub.items_at_depth(d-1))
        return result
    ...
    def insert(self, item: Any) -> None:
        ...
        True
        """
        # Revisit how to use the function randint, if needed. For example:git
        # >>> random.randint(1, 3)
        ...
        if not self._root:
            ...
            if decide == 3:
                ...
                #pick one of the existing subtrees at random,
                #and *recursively insert* the new item into that subtree.
                decide = random.randint(0, len(self._subtrees)-1)
                self._subtrees[decide].insert(item)
