...
class CallHistory:
    ...
    def register_outgoing_call(self, call: Call) -> None:
        ...
        """
        # If there exists calls made this time
        mth = call.time.month
        yr = call.time.year
        # If exist within dictionary
        if (mth, yr) in self.outgoing_calls:
            self.outgoing_calls[(mth, yr)].append(call)
        else:
            self.outgoing_calls[(mth, yr)] = [call]
    ...
    def register_incoming_call(self, call: Call) -> None:
        ...
        mth = call.time.month
        yr = call.time.year
        # If calls made at present exist within dictionary
        if (mth, yr) in self.incoming_calls:
            self.incoming_calls[(mth, yr)].append(call)
        else:
            # If not, create key with current month, year
            self.incoming_calls[(mth, yr)] = [call]
