...
class CustomerFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        """
        data2 = []
        # Check if input exists in customer list
        try:
            for c in customers:
                if c.get_id() == int(filter_string):
                    for call in data:
                        if (call.src_number in c.get_phone_numbers()
                                or call.dst_number in c.get_phone_numbers()):
                            data2.append(call)
                    return data2
        except ValueError:
            return data
...
class DurationFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        Do not mutate any of the function arguments!
        """
        if filter_string[0:1] != 'G' and filter_string[0:1] != 'L':
            return data
        else:
            try:
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        """
        data2 = []
        # Input coordinates, restrictions box
        coords = filter_string.split(', ')
        try:
            for c in data:
                # set lower, left, up, right bounds
                lob = min(c.src_loc[0], c.dst_loc[0])
                lb = min(c.src_loc[1], c.dst_loc[1])
                ub = max(c.src_loc[0], c.dst_loc[0])
                rb = max(c.src_loc[1], c.dst_loc[1])
                if float(coords[0]) < lob and float(coords[1]) < lb\
                        and float(coords[2]) > ub and float(coords[3]) > rb:
                    data2.append(c)
