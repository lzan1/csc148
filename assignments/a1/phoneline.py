...
class PhoneLine:
    ...
    def make_call(self, call: Call) -> None:
        ...
        # bill it according to the contract for this phone line
        if (call.time.month, call.time.year) in self.bills:
            self.contract.bill_call(call)
        else:
            # If no bill for this cycle, create new bill
            bill = Bill()
    ...
    def receive_call(self, call: Call) -> None:
        ...
        # If current time is not a key in bills, no bill for current cycle
        if (call.time.month, call.time.year) not in self.bills:
            # Therefore, create new month
            bill = Bill()
            self.contract.new_month(call.time.month, call.time.year, bill)
