...
def process_event_history(log: dict[str, list[dict]],
    ...
    for vent in log['events']:
        # Preparing to instantiate only 'call' objects
        if vent['type'] == 'call':
            # Variables for long arugments
            srcnum = vent['src_number']
            dstnum = vent['dst_number']
            time = datetime.datetime.strptime(vent['time'], "%Y-%m-%d %H:%M:%S")
            duration = vent['duration']
            srcloc = vent['src_loc']
            dstloc = vent['dst_loc']

            # Advance billing month into present month
            if time.month != billing_month:
                billing_month = time.month
                # Advance each customer into new month
                for cuss in customer_list:
                    cuss.new_month(billing_month, billing_date.year)

            cl = Call(srcnum, dstnum, time, duration, srcloc, dstloc)
            # Find caller/callee by number
            src_c = find_customer_by_number(srcnum, customer_list)
            dst_c = find_customer_by_number(dstnum, customer_list)
            # If customers exist, store the call in their records
            if src_c and dst_c:
                src_c.make_call(cl)
                dst_c.receive_call(cl)
